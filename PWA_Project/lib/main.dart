...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Uint8List? _poachingImageBytes;
  String? _poachingImageName;
  ...
  // Pick or capture photo for poaching incident
  Future<void> _pickPoachingImage() async {
    try {
      ...
      if (image != null) {
        ...
        setState(() {
          _poachingImageBytes = bytes;
          _poachingImageName = image.name;
        });
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _submitData() async {
    ...
    if (_selectedCategory == 'Sighting') {
      ...
      if (_selectedIncident == 'Poaching' && _selectedPoachingType != null) {
        ...
        if (_poachingImageBytes != null) {
          data['poaching_image'] = base64Encode(_poachingImageBytes!);
          data['poaching_image_name'] = _poachingImageName;
        }
      }
    ...
    setState(() {
      ...
      _poachingImageBytes = null;
      _poachingImageName = null;
    });
              ...
              if (_showForm) ...[
                ...
                if (_selectedCategory == 'Sighting') ...[
                  ...
                  if (_selectedIncident == 'Poaching') ...[
                    ...
                    // Photo upload button (only show if poaching type is selected)
                    if (_selectedPoachingType != null) ...[
                      const SizedBox(height: 16),
                      ElevatedButton.icon(
                        onPressed: _pickPoachingImage,
                        icon: const Icon(Icons.camera_alt),
                        label: Text(_poachingImageBytes == null ? 'Add Photo (Optional)' : 'Photo Added ✓'),
                        style: ElevatedButton.styleFrom(
                          minimumSize: const Size(double.infinity, 50),
                          backgroundColor: _poachingImageBytes != null ? Colors.green : null,
                        ),
                      ),
                      if (_poachingImageBytes != null) ...[
