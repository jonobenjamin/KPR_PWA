...
async function verifyBearerUser(req, res, next) {
...
async function optionalBearerUser(req, res, next) {
  const authHeader = req.headers.authorization || '';
  const token = authHeader.startsWith('Bearer ') ? authHeader.slice(7).trim() : '';
...
router.get('/recent', async (req, res) => {
  try {
    ...
    const snapshot = await db.collection('observations').orderBy('timestamp', 'desc').limit(500).get();
...
router.get('/', async (req, res) => {
  try {
    ...
    snapshot.forEach(doc => {
      const data = doc.data();
      // Return full record - all fields from the app (for map display and admin)
      const observation = {
        id: doc.id,
        ...data
      };
      // Exclude large base64 image data from response (images accessed via /api/observations/:id/image)
      delete observation.poaching_image;
...
router.post('/', optionalBearerUser, upload.single('image'), async (req, res) => {
  try {
    // Check if Firebase is initialized
    if (!db) {
      return res.status(503).json({
    ...
    if (req.firebaseUid) {
      observationData.user_uid = req.firebaseUid;
    }
    ...
    if (category === 'Sighting') {
      ...
      if (note !== undefined && note !== null && String(note).trim() !== '') {
        observationData.note = String(note).trim().slice(0, 2000);
      }
      if (sighting_count !== undefined && sighting_count !== null && String(sighting_count).trim() !== '') {
        const n = parseInt(String(sighting_count), 10);
        if (!Number.isNaN(n) && n > 0) {
          observationData.sighting_count = Math.min(n, 999);
        }
      }
      ...
      if (vulture_tagged !== undefined) observationData.vulture_tagged = vulture_tagged;
    }
    ...
    } else if (req.body.sighting_image && req.body.sighting_image_name) {
      console.log('📸 Sighting image (base64 JSON)');
      try {
        const base64Data = req.body.sighting_image.replace(/^data:image\/[a-z]+;base64,/, '');
        imageBuffer = Buffer.from(base64Data, 'base64');
        imageMimeType = 'image/jpeg';
        imageOriginalName = req.body.sighting_image_name;
      } catch (decodeError) {
        console.error('❌ Failed to decode sighting image:', decodeError.message);
      }
    }
